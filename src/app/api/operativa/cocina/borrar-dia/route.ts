...
import { eliminarComandasCocinaDelDia } from "@/lib/operativa/operativa-server";
...
export async function POST(request: Request) {
  ...
  try {
    const eliminadas = await eliminarComandasCocinaDelDia(
      request.headers.get("authorization")!.slice(7),
      fecha,
    );
    return NextResponse.json({ eliminadas });
  } catch (error) {
    console.error("[operativa/cocina/borrar-dia]", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "No se pudieron borrar las comandas del día",
      },
      { status: 500 },
    );
  }
