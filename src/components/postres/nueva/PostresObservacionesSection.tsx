...
import { OBSERVACIONES_POSTRES_RAPIDAS } from "@/data/postres-catalogo";
import { Button } from "@/components/ui/Button";
import { Chip } from "@/components/ui/Chip";
import { SectionCard } from "@/components/ui/SectionCard";

interface PostresObservacionesSectionProps {
...
export function PostresObservacionesSection({
  observaciones,
  onChange,
  onAdd,
  onRemove,
  onRapida,
}: PostresObservacionesSectionProps) {
  return (
    <SectionCard
      title="Observaciones"
      actions={
    ...
    >
      <div className="mb-3 flex flex-wrap gap-2">
        {OBSERVACIONES_POSTRES_RAPIDAS.map((obs) => (
          <Chip key={obs} label={obs} onClick={() => onRapida(obs)} size="sm" />
        ))}
      </div>
